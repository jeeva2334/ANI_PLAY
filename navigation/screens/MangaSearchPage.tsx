...
const MangaSearchPage = ({navigation}) => {
    const [searchBox,setSearchBox] = useState('')
    const [searched,setSearched] = useState(false)
    const [loading,setLoading] = useState(false)
    const [sp,setSp] = useState('')
    const {searchManga,resultManga,setresultManga} = useContext(AnimeProvider)
    const fetch =  async () => {
        setLoading(true)
        searchManga(searchBox)
        setSearched(true)
    ...
    return (
        <SafeAreaView style={styles.container}>
            ...
            <ScrollView style={{width:"100%"}} showsVerticalScrollIndicator={false}>
                    {searched === true && resultManga.length !== 0 ? resultManga.map((item)=>(<TouchableOpacity style={styles.card} onPress={()=>navigation.navigate('details',{id:item.id})}>
                    ...
                    <SafeAreaView style={{paddingLeft:6,width:"60%"}}>
                        ...
                        <View style={styles.status}>
                            ...
                            <Text style={styles.para}>{item.status}</Text>
                        </View>
                        <View style={styles.status}>
